...
export const CourseDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'instructor' | 'homework' | 'faq'>('overview');
  const {
    course,
    courseClassrooms,
    selectedClassroomId,
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="relative bg-gradient-to-br from-primary-600 via-primary-700 to-secondary-600 overflow-hidden">
        ...
        <div className="container-custom py-12 relative z-10">
          <Link
            to="/courses"
            className="inline-flex items-center gap-2 text-white/90 hover:text-white transition-colors mb-10 group"
          >
            <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
            <span className="text-sm font-medium">{t('common.backToCourses')}</span>
          </Link>
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1440 60" className="w-full h-12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 60L60 55C120 50 240 40 360 35C480 30 600 30 720 32.5C840 35 960 40 1080 42.5C1200 45 1320 45 1380 45L1440 45V60H1380C1320 60 1200 60 1080 60C960 60 840 60 720 60C600 60 480 60 360 60C240 60 120 60 60 60H0Z" fill="currentColor" className="text-gray-50 dark:text-gray-900" />
          </svg>
        </div>
      </div>
            ...
            {courseFeatures.map((feature, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Icon container */}
                  <div className={`relative w-16 h-16 rounded-2xl bg-gradient-to-br ${feature.gradient} flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-xl`}>
                  ...
                  {/* Decorative corner */}
                  <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${feature.gradient} opacity-0 group-hover:opacity-5 rounded-bl-full transition-opacity duration-500`} />
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {tabs.map((tab) => {
              ...
              return (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as 'overview' | 'instructor' | 'homework' | 'faq')}
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            {activeTab === 'overview' && (
              <>
                    ...
                    <div className="prose prose-lg dark:prose-invert max-w-none">
                      <div className="bg-gradient-to-r from-primary-50/80 to-secondary-50/80 dark:from-primary-900/20 dark:to-secondary-900/20 rounded-2xl p-8 border-l-4 border-primary-500 shadow-inner">
                        <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-lg whitespace-pre-line">
                          {currentLang === 'vi' && course.descriptionVi ? course.descriptionVi : 
                           currentLang === 'en' && course.descriptionEn ? course.descriptionEn : 
                           course.description}
                        </p>
                      </div>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 p-10 hover:shadow-2xl transition-all duration-300 overflow-hidden relative">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="grid md:grid-cols-2 gap-5">
                      {[
                        { icon: BookOpen, text: t('courseDetail.getMaterials') },
                        { icon: Users, text: t('courseDetail.getSupport') },
                        { icon: Globe, text: t('courseDetail.getBilingual') },
                        { icon: Clock, text: t('courseDetail.getFlexible') },
                      ].map((item, idx) => {
                        const Icon = item.icon;
                        return (
                          <div key={idx} className="flex items-center gap-4 p-5 rounded-2xl bg-gradient-to-br from-gray-50 to-gray-100/50 dark:from-gray-700/50 dark:to-gray-700/30 hover:from-primary-50 hover:to-secondary-50 dark:hover:from-primary-900/20 dark:hover:to-secondary-900/20 transition-all duration-300 group border border-gray-200/50 dark:border-gray-600/50 hover:border-primary-200 dark:hover:border-primary-700 hover:shadow-lg">
                ...
                {/* Classrooms Section */}
                {courseClassrooms && courseClassrooms.length > 0 && (
                  <ClassroomsSection
                    courseClassrooms={courseClassrooms}
                  ...
                  <div className="bg-gradient-to-br from-blue-50/80 to-cyan-50/80 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-2xl p-8 border border-blue-100 dark:border-blue-800 shadow-inner">
                    <div className="space-y-6">
                      ...
                      <div className="grid sm:grid-cols-3 gap-4 pt-6 border-t border-blue-200 dark:border-blue-700">
                        {[
                          { icon: Award, text: t('courseDetail.instructorBed') },
                          { icon: Star, text: t('courseDetail.instructorTesol') },
                          { icon: CheckCircle2, text: t('courseDetail.instructorIelts') },
                        ].map((item, idx) => {
                          const Icon = item.icon;
                          return (
                            <div key={idx} className="flex items-center gap-3 p-4 rounded-xl bg-white dark:bg-gray-800 shadow-md hover:shadow-lg transition-all duration-300 group border border-gray-200 dark:border-gray-700 hover:border-primary-300 dark:hover:border-primary-600">
            ...
            <div className="lg:sticky lg:top-36 space-y-6">
              ...
              {course.type === 'free_youtube' && (
                <>
                  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-8">
                    ...
                    {course.youtubePlaylistUrl && (
                      <a
                        ...
                        className="w-full inline-flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold px-6 py-3 rounded-xl transition-all shadow-lg hover:shadow-xl hover:scale-105"
                      >
                  ...
                  {activeTab !== 'faq' && <ContactInfo variant="compact" />}
                </>
              )}
